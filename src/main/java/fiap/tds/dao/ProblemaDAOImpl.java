...
public class ProblemaDAOImpl implements ProblemaDAO {
    ...
    public void inserir(Problema problema) throws SQLException {
        String sql = "INSERT INTO TB_PROBLEMA (id_problema, id_questionario, nivel_de_prioridade, descricao) VALUES (?, ?, ?, ?)";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, problema.getIdProblema());
            stmt.setInt(2, problema.getQuestionario().getIdQuestionario());
            stmt.setInt(3, problema.getNivel_de_prioridade());
            stmt.setString(4, problema.getDescricao());
            stmt.executeUpdate();
        }
    }
    ...
    public List<Problema> listarTodos() throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                problema.setNivel_de_prioridade(rs.getInt("prioridade"));
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    problema.setIdProblema(rs.getInt("id_problema"));
                    problema.setNivel_de_prioridade(rs.getInt("prioridade"));
                    problema.setDescricao(rs.getString("prioridade"));
                }
            }
        }
        return problema;
